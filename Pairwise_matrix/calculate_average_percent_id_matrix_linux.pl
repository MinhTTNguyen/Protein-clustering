...
mkdir "Temp" || die "Unable to create directory <$!>\n";
chmod 0777, "Temp";
...
#my @subs1=("sub9","sub45","sub11","sub12","sub14","sub15","sub17","sub18","sub20","sub26","sub27","sub28","sub29","sub30","sub31","sub35","sub36","sub46","sub49");
...
sub Pairwise_id
{
	...
	my $seqIO_obj = Bio::SeqIO -> new(-file => ">Temp\/temp.fasta", -format => 'fasta');
	$seqIO_obj->write_seq($seq_obj1);
	$seqIO_obj->write_seq($seq_obj2);

	my @params = ('matrix' => 'BLOSUM', 'type' => 'protein', 'outfile' => "Temp\/MSA_temp.fasta", 'output' => 'fasta');
	my $factory = Bio::Tools::Run::Alignment::Clustalw->new(@params);
	my $simple_aln_obj = $factory-> align("Temp\/temp.fasta", -format => 'fasta');
	my $pairwise_percent_identity=$simple_aln_obj->overall_percentage_identity;

	unlink("Temp/MSA_temp.fasta");
	unlink("Temp/temp.fasta");
