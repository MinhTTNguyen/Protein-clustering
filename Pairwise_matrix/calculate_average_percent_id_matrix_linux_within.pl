...
mkdir "Temp_within" || die "Unable to create directory <$!>\n";
chmod 0777, "Temp_within";
...
my $filein="GH24_CLANcluster_selected_proteins_domainseq.fasta";
my $fileout=substr($filein,0,-6);
$fileout=$fileout."_percent_id_matrix_within.txt";
...
#my @subs1=("sub1","sub2","sub3","sub4","sub5");
...
sub Percent_identity_2_subs
{
	...
	my $average_pairwise_subs="";
	if ($total_pairs==0){$average_pairwise_subs="NA";}
	else{$average_pairwise_subs=$total_percent_id/$total_pairs;}
...
sub Pairwise_id
{
	...
	my $seqIO_obj = Bio::SeqIO -> new(-file => ">Temp_within\/Temp_within.fasta", -format => 'fasta');
	$seqIO_obj->write_seq($seq_obj1);
	$seqIO_obj->write_seq($seq_obj2);

	my @params = ('matrix' => 'BLOSUM', 'type' => 'protein', 'outfile' => "Temp_within\/MSA_Temp_within.fasta", 'output' => 'fasta');
	my $factory = Bio::Tools::Run::Alignment::Clustalw->new(@params);
	my $simple_aln_obj = $factory-> align("Temp_within\/Temp_within.fasta", -format => 'fasta');
	my $pairwise_percent_identity=$simple_aln_obj->overall_percentage_identity;

	unlink("Temp_within/MSA_Temp_within.fasta");
	unlink("Temp_within/Temp_within.fasta");
